...
void freeCache(hashTable *cache)
{
    if(cache == NULL || cache->tableBoxes == NULL) return;

    for(int i = 0; i < cache->capacity; i++)
    {
        hashNode *currentHashNode = cache->tableBoxes[i];
        while(currentHashNode)
        {
            hashNode *tempHashNode = currentHashNode;
            currentHashNode = currentHashNode->next;
            free(tempHashNode);
        }
    }
    free(cache->tableBoxes);

    CacheNode *currentCacheNode = MRU;
    while(currentCacheNode)
    {
        CacheNode *tempCacheNode = currentCacheNode;
        currentCacheNode = currentCacheNode->next;
        free(tempCacheNode->token);
        free(tempCacheNode);
    }
